...
Automatisch generiert von SnipX AI`
  ...
  const handleGenerateThumbnail = async () => {
    ...
    try {
      ...
      if (isAuthenticated && uploadedVideoId) {
        ...
        const thumbnailUrl = ApiService.getVideoThumbnailUrl(uploadedVideoId, selectedFrameIndex);
  ...
  const handleDownloadVideo = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://fyp-project-production-a4a3.up.railway.app/api/videos/${uploadedVideoId}/download`, {
        method: 'GET',
        headers: {
